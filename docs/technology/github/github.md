...
<br>
##### 2.将本地项目与远程仓库关联
打开你的项目所在的本地文件夹，在终端（或命令行）中执行以下命令：<br>
初始化本地 Git 仓库 (如果尚未初始化): bash终端符> git init<br>
添加远程仓库地址:<br>
在 GitHub 仓库页面，点击绿色的 Code 按钮，复制 HTTPS 或 SSH 地址。<br>
...
git bash终端执行:<br>
git add .  #载入变更<br>
git commit -m "更新了文档"  #填写变更说明<br>
git push  #推送变更 （因为之前用 -u 关联过，所以现在可以直接推送）<br>
<br>
<br>

### 拉取同步
如果直接在 GitHub 网页上修改了内容（例如编辑了文件、合并了 Pull Request 等），<br>
这些更改会反映在远程仓库中，而本地仓库并没有自动同步这些变化。<br>
为了保持本地仓库与远程仓库的一致性，需要手动从远程仓库拉取（Pull）最新的更改。<br>

在本地项目目录中打开终端或命令行工具（Git Bash），然后运行以下命令：<br>
git pull
<br>
<br>
